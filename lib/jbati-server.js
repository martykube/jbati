...
(function() {
	...
	SqlMapClient.prototype = {
		...
		// Execute a select statement for one returned object.
		//
		...
		// method: queryForList
		// Execute a select statement for more than one returned object.
		//
		queryForList: function(statementId, parameterObject) {
			...
			if(resultSet.hasData) {
				resultSet.rowsAsArrays.forEach(function(row) {
					var newObject = eval('new ' + statement.@resultClass + '()');
					for(var i = 0; i <	resultSet.columns.length; i++) {
						newObject[resultSet.columns[i]] = row[i];
					}
	...
	ParameterMapper.bind = function(sql, parameterObject) {
		...
		Server.log.trace('bind: sql: ' + sql);
		Server.log.trace('bind: parameterObject: ' + 
		Jaxer.Serialization.toJSONString(parameterObject));
		var pm = new ParameterMapper();
		var replacementTokenRegex = new RegExp('#([^#]+)#', 'g'); 
		var matches;
		var copyFrom = 0;
		while(null != (matches = replacementTokenRegex.exec(sql))) {
			Server.log.trace('bind: replacing: ' + matches[1]);
